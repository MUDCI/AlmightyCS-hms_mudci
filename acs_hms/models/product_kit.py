...
class ACSProductKitLine(models.Model):
    ...
    uom_id = fields.Many2one(related='product_id.uom_id' , string="Unité de mesure", readonly=True)
    ...
    unit_price = fields.Float(related='product_id.list_price', string='Prix du produit')
    # standard_price = fields.Float(related='product_id.standard_price', string='Cost Price')
    standard_price = fields.Float(related='product_id.standard_price', string='Prix de revient')
    # total_price = fields.Float(compute=_get_total_price, string='Total Price')
    total_price = fields.Float(compute=_get_total_price, string='Prix total')
    # total_standard_price = fields.Float(compute=_get_total_price, string='Total Cost Price')
    total_standard_price = fields.Float(compute=_get_total_price, string='Prix de revient total')
...
class ProductTemplate(models.Model):
    ...
    is_kit_product = fields.Boolean("Kit Produit", help="Adding this product will lead to component consumption when added in medical flow")
