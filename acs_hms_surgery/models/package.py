...
class AcsHmsPackage(models.Model):
    ...
    amount_untaxed = fields.Monetary(string='Montant non imposable ', store=True, readonly=True, compute='_amount_all', tracking=True)
...
class AcsHmsPackageLine(models.Model):
    ...
    product_id = fields.Many2one('product.product', string='Produit', domain=[('sale_ok', '=', True)], change_default=True, ondelete='restrict')
    # product_id = fields.Many2one('product.product', string='Product', domain=[('sale_ok', '=', True)], change_default=True, ondelete='restrict')
    product_uom_qty = fields.Float(string='Quantité', digits=('Product Unit of Measure'), default=1.0)
    # product_uom_qty = fields.Float(string='Quantity', digits=('Product Unit of Measure'), default=1.0)
    product_uom_category_id = fields.Many2one('uom.category', related='product_id.uom_id.category_id')
    product_uom_id = fields.Many2one('uom.uom', string='Unité de mesure', domain="[('category_id', '=', product_uom_category_id)]")
